...
export class PaymentsService {
  ...
  private readonly returnBase = 'https://b46bc20757e5.ngrok-free.app';
  ...
  async createPreferenceFromOrder(orderId: string | number) {
    ...
    const items = details.map((d, idx) => {
      ...
      return {
        ...
        unit_price: unit,
        currency_id: 'ARS',
        orderDetailId: (d as any).id ?? null,
        orderDetailName: d.nameProduct,
      };
    });
    ...
    const pref = await this.preference.create({
      body: {
        ...
        back_urls: {
          success: `${this.returnBase}/success`,
          failure: `${this.returnBase}/failure`,
          pending: `${this.returnBase}/pending`,
        },
    ...
    return {
      ...pref,
      _payment: { id: savedPayment.id, total, currency: 'ARS' },
    };
  }

  async setPaymentResultByPreferenceId(preferenceId: string, status: 'paid' | 'payment_failed', paymentId?: string) {
